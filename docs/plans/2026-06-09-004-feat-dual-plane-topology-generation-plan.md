...
- KTD2. **端口分配是新建逻辑，不套用 generic 的定偏移方案。** generic 用 `switch_interconnect_port_offset = end_systems_per_switch` 的固定偏移；dual-plane 每台 switch 承接 不定数量接入链路（可能跨 group）+ 平面内骨干链路，需 first-free 端口分配（按确定性顺序逐链路取下一个空闲端口）。
- KTD3. **确定性排序显式规范（origin R5）。** 节点序 = switches（按 group 顺序、A 平面先于 B），再 end systems（按 group、再声明序）；链路序 = 接入链路（每 ES 先 primary 后 backup）→ 平面内 backbone（按平面 A/B、group 序）。numeric_id 按此序递增，落 `sort_*` 后稳定。
...
- KTD6. **`allocation` / `switches[].role` 可选、生成器用确定性默认。** 端口 first-free、布局 A/B 双行 grid；不要求调用方传，descriptor 不强制列。
...
**Files**：
- `src-tauri/src/topology_compute.rs`（新增 DualPlaneParams serde 结构、`validate_dual_plane_params`、`create_dual_plane_redundant_topology`；改 `initialize_topology` dual-plane 分支；内联 `#[cfg(test)]` 测试）
- `src-tauri/src/topology_intermediate.rs`（仅在需要新 helper 时；优先复用既有 `create_ports`/`create_link`/`derive_mac_address`/`sort_*`）

**Approach**：
- 新增 serde 结构镜像 origin 参数契约：`planes`(A/B)、`switches`{id,plane,groupId,portCount?,role?}、`switchGroups`{id,planeSwitches{A,B}}、`endSystems`{id,groupId,attachment{primary{switchId,plane},backup{switchId,plane}}}、`backbone`{mode,withinPlane}、`crossPlaneLinks`{mode}、`allocation?`。用 `#[serde(default)]` + 缺字段在校验层报错（不靠 serde 失败）。
...
**Execution note**：校验与生成先写失败的 Rust 单测（按 AE1-5）再实现。
...
- Covers AE5. group 只引用一个平面 / ES 引用不存在 switchId / 缺 `backbone` / 缺 `crossPlaneLinks` → 各自可定位结构化错误（断言 error path）。
- R5 确定性：同一参数生成两次，nodes/links/ports/positions 完全相等（含 numeric_id 序）。
- 端口不足：显式 `portCount` 小于实际需求 → 端口不足错误。
...
**Requirements**：R7（contract honesty）, R3。
...
- `src-node/mcp/topology-tools.ts`（`dualPlaneParamsSchema`：`backbone.mode` `z.literal("line")`、`crossPlaneLinks.mode` `z.literal("none")`，`withinPlane` 维持 `z.literal(true)`）
- `src-node/mcp/topology-tools.test.ts`（更新/补 zod 测试）

**Approach**：descriptor 是 JSON 字面，改 itemShape 字符串即可；zod 把 `z.enum([...])` 换 `z.literal(...)`。若存在 catalog↔zod 一致性 drift 测试，同步更新其期望。注释标注 ring/paired 待实现时 re-advertise。
...
**Dependencies**：U1（生成已可用后才解禁，避免误导 agent）。
