...
流表的 `paths` 列统一 JSON 形状为**裸数组**（castup 求解时应消费此列作为路径约束）：

```json
[ { "node_path": ["4","0","2","6"], "link_seqs": [0, 8, 14] } ]
```

- 语义统一为**路径凭证**：消费前复验，有效即**必须按此路径排程**（用户指定的绕路在拓扑不变时稳定沿用）；拓扑变更导致失效时静默重推导并在规划期自动刷新回写。
- ST/BE 恒 1 条；RC 流恒 2 条（`[0]`=A 平面、`[1]`=B 平面，802.1CB 不相交双路径）。
- `NULL`：未沉淀（历史存量）——求解器可按最短路（我方语义：唯一最短路，等长多路径在录入期已被拒绝或已由用户指定消歧；存量 NULL 流规划一次即回写沉淀凭证）。
- `node_path` 为节点 mid 序列（含首尾），`link_seqs` 为拓扑链路序号序列（`topology_links.link_seq`）。
- 读侧兼容历史形状 `{"version":1,"origin":...,"routes":[...]}` 与更旧 RC `{"a":{...},"b":{...}}`（都按裸数组语义消费，不回写迁移）。
...
3. 全部时间量 ns 整数
4. 有效路径凭证（paths 列）与 RC 双路径必须遵守，不可改 route
