...
- **W 随同步参数缩放**。`W = N × max(syncInterval, pdelayInterval) + 下限`，不写死绝对值——用户调大同步周期时 W 自动跟上。计算须用**生效值**（用户把 sync/pdelay 留空时，用 INET 默认值算 W，与 bundle 实际生效的一致）。
...
- 假设 gPTP 实际收敛时刻稳定落在 `N × max(syncInterval, pdelayInterval) + 下限` 之内。这是 R2 校准的核心待验假设，用真机 gPTP 收敛数据坐实。
