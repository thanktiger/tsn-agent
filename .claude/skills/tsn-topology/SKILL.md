...
增量加链路（`apply_operations` 的 `link_add`）也带链路速率字段 `speed`（同样整数 Mbps）：**按用户意图填**（用户说"千兆/万兆/百兆"就填 1000/10000/100），用户没提就省略——后端缺省落库 1000，与 initialize 模板一致，保证手加链路和初始链路速率口径统一。
