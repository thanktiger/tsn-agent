...
//! 内容哈希作无害冗余收录。
//!
//! 发版 checklist：每次发布把当期出厂文件的 sha256 追加到此表（旧值永不删除），
//! 使后续版本能把「未编辑的旧出厂文件」静默升级为新内容。

/// 历代出厂内容 sha256（hex，小写）。命中此表即视为「未被用户编辑的出厂内容」。
pub const HISTORICAL_FACTORY_HASHES: &[&str] = &[
    // —— feat/release-writable-skill 播种内容（33a28d9）——
    // tsn-topology/SKILL.md（单体版，场景拆分前）
    "6daa182bcd7ed3a73cb561542218024f2fdf23e247c1b1ac4bb2d7f782702a81",
    // tsn-topology/package.json（{"type":"commonjs"} 残留）
