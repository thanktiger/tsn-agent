...
/// 只读查询内核：flow_plans 全行 + topology_nodes.name 显示名映射 + 类别计数。
pub async fn get_flow_plan_inner(
    ...
    let rows = sqlx::query(
        "SELECT p.node, n.name AS node_name, p.eth_n, p.gate_index, p.initially_open, p.offset_ns, p.durations_ns, p.solver \
         ...
         WHERE p.session_id = ? ORDER BY p.node, p.eth_n, p.gate_index",
    )
    .bind(session_id)
    .fetch_all(pool)
    .await
    ...
    let entries = rows
        ...
        .map(|r| {
            if solver.is_none() {
                solver = Some(r.get::<String, _>("solver"));
            }
            ...
            FlowPlanEntry {
                ...
                durations_ns: serde_json::from_str(&durs).unwrap_or_default(),
            }
        })
        .collect();
...
"#;
    ...
    fn get_flow_plan_returns_entries_names_and_counts() {
        tauri::async_runtime::block_on(async {
            ...
            for (node, eth_n, offset, durs) in [
                ("0", 1i64, 29_470i64, "[4560,995440]"),
                ("2", 0i64, 0i64, "[300000,700000]"),
            ] {
                sqlx::query("INSERT INTO flow_plans (session_id, stream_seq, node, eth_n, gate_index, initially_open, offset_ns, durations_ns, solver) VALUES ('s1', 0, ?, ?, 7, 1, ?, ?, 'Z3')")
                    .bind(node).bind(eth_n).bind(offset).bind(durs)
                    .execute(&pool).await.unwrap();
            }
