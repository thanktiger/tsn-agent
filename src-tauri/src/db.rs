...
/// flow_streams 扩展列迁移（2026-07-13，流面板重设计 S4/R17）：老库补 5 个设备级流标识列
/// （源/目 MAC、VLAN ID、发送偏移预留槽）；新库由 FLOW_DOMAIN_SCHEMA_SQL 的 CREATE 直接带列，
/// 此处 no-op。命令式 pragma 守卫范式（同 ensure_topology_nodes_name_column）。
pub async fn ensure_flow_streams_extended_columns(
    ...
    for (col, col_type) in cols {
        ...
        if exists == 0 {
            sqlx::query(&format!(
                "ALTER TABLE flow_streams ADD COLUMN {col} {col_type}"
            ))
            .execute(pool)
            .await?;
        }
...
pub const SESSION_SCOPED_TABLES: &[(&str, &[&str])] = &[
    ...
    (
        ...
        &[
            ...
            "dst_mac",
            "vlan_id",
            "earliest_send_offset_ns",
            "latest_send_offset_ns",
        ],
    ),
    (
        "flow_plans",
